...
`cardPrewiew:clickButton` - клик по кнопке внутри подробной карточки товара.
...
`productCatalog:showProducts` - сохранение в модели данных всего списка товаров.
...
`events.on("productCatalog:showProducts", () => void)` - генерирует все карточки товара и выводит их отображение в галерее.
...
`events.on("cardBasket:delete", (item: IProduct) => void)` - удаляет товар, переданный вместе с генерацией события, из корзины покупателя.
`events.on("basket:order", () => void)` - вызывает метод валидации данных модели данных и выводит в модальном окне форму с адресом и способом оплаты.
...
`events.on("cardPrewiew:clickButton", (obj: IPrewiewClick) => void)` - в зависимости от переданного параметра добавляет карточку в корзину или удаляет выбранную карточку из корзины, закрывает модальное окно.