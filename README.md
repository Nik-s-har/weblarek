...
#### Интерфейс IForm
Интерфейс для типизации дженерика при вызове метода render(data?: Partial<T>): HTMLElement родительского класса Component внутри класса Form.

Свойства интерфейса:
`submit: boolean` -  содержит логическую переменную, которая определяет доступность кнопки submite формы.
`errors: string` - содержит сообщение об ошибке.
...
Методы класса:
`set sybmiteActive(switch: boolean)` - сеттер устанавливает кнопку submite формы в активное, или неактивное состояние.
`set errors(value: string)` - сеттер выводит сообщение об ошибке при заполнении формы.

#### Интерфейс IFormOrder
Интерфейс для типизации дженерика при вызове метода render(data?: Partial<T>): HTMLElement родительского класса Component внутри класса FormOrder. Является расширением интерфейса IForm.
...
Интерфейс для типизации дженерика при вызове метода render(data?: Partial<T>): HTMLElement родительского класса Component внутри класса FormContacts. Является расширением интерфейса IForm.
...
`productCatalog:clearSelectedProduct` - очищено поле выбранной карточки.
...
`events.on("cardPrewiew:addCard", () => void)` - добавляет карточку в корзину и удаляет выбранную карточку из модели данных.
`cardPrewiew:delCard` - удаляет карточку товара из корзины и удаляет выбранную карточку из модели данных.
`events.on("shoppingCart:change", () => void)` - обновляет счетчик товаров в корзине.
